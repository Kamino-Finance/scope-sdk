...
export function getRebalanceRawEncoder(): FixedSizeEncoder<RebalanceRawArgs> {
  return getStructEncoder([
    ["params", fixEncoderSize(getBytesEncoder(), 128)],
    ["state", fixEncoderSize(getBytesEncoder(), 256)],
    ["referencePriceType", getU8Encoder()],
  ]);
}

export function getRebalanceRawDecoder(): FixedSizeDecoder<RebalanceRaw> {
  return getStructDecoder([
    ["params", fixDecoderSize(getBytesDecoder(), 128)],
    ["state", fixDecoderSize(getBytesDecoder(), 256)],
    ["referencePriceType", getU8Decoder()],
  ]);
}

export function getRebalanceRawCodec(): FixedSizeCodec<
  RebalanceRawArgs,
  RebalanceRaw
> {
