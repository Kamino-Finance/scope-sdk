...
export const TERMS_SIGNATURE_DISCRIMINATOR = new Uint8Array([
  197, 173, 136, 91, 182, 49, 113, 19,
]);

export function getTermsSignatureDiscriminatorBytes() {
  return fixEncoderSize(getBytesEncoder(), 8).encode(
    TERMS_SIGNATURE_DISCRIMINATOR,
  );
}
...
export function getTermsSignatureEncoder(): FixedSizeEncoder<TermsSignatureArgs> {
  return transformEncoder(
    ...
    (value) => ({ ...value, discriminator: TERMS_SIGNATURE_DISCRIMINATOR }),
  );
}
...
export function getTermsSignatureCodec(): FixedSizeCodec<
  TermsSignatureArgs,
  TermsSignature
> {
...
export function decodeTermsSignature<TAddress extends string = string>(
  encodedAccount: EncodedAccount<TAddress>,
): Account<TermsSignature, TAddress>;
export function decodeTermsSignature<TAddress extends string = string>(
  encodedAccount: MaybeEncodedAccount<TAddress>,
): MaybeAccount<TermsSignature, TAddress>;
export function decodeTermsSignature<TAddress extends string = string>(
  encodedAccount: EncodedAccount<TAddress> | MaybeEncodedAccount<TAddress>,
): Account<TermsSignature, TAddress> | MaybeAccount<TermsSignature, TAddress> {
  return decodeAccount(
    encodedAccount as MaybeEncodedAccount<TAddress>,
    getTermsSignatureDecoder(),
  );
}
...
export async function fetchMaybeTermsSignature<
...
export async function fetchAllTermsSignature(
  ...
  const maybeAccounts = await fetchAllMaybeTermsSignature(
    rpc,
    addresses,
    config,
  );
...
export async function fetchAllMaybeTermsSignature(
  ...
  return maybeAccounts.map((maybeAccount) =>
    decodeTermsSignature(maybeAccount),
  );
}
